...
# rename columns
sg1_top_samples = ["Feb7-1", "Feb7-3", "Feb7-6", "Feb7-9"]
sg1_bot_samples = ["Feb7-12", "Feb7-15", "Feb7-20", "Feb7-61", "Feb7-65", "SG1-E-mid"]
sg1=1
sg2=1
for col in df:
	if col in site1:
		if col in sg1_top_samples:
			new_name = "SG1_TOP_"+col
		elif col in sg1_bot_samples:
			new_name = "SG1_BOT_"+col
		else:
			new_name = "SG1_0_"+col
		sg1+=1
	elif col in site2:
		new_name = "SG2_"+col
		sg2+=1
	#if "TOP" in new_name or "BOT" in new_name: 
	df[new_name]=df[col]
...
for sample in df.columns.values:
	print sample
	if sample.split('_')[-1] in sg1_top_samples: c="m"
	elif sample.split('_')[-1] in sg1_bot_samples: c="gold"
	elif "SG1" in sample: c="r"
	elif "SG2" in sample: c="b"
	else: c="w"
	col_colors.append(c)
row_colors=[]
...
g = sns.clustermap(df, figsize=(8,8), col_colors=col_colors, col_cluster=False, yticklabels=False, xticklabels=True, cmap="magma_r")
